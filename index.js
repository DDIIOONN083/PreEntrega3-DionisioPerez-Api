 ...
 * @Last Modified time: 2023-08-31 21:16:27
 */
		        ...
		        if (estaPresente) {
		        	const products = carrito.map(producto => {
		        		if (producto.titulo === infoProductos.titulo) {
		        			/*  alert("Ya estas inscripto en esa materia");  */
...
/* PUEDO SUMAR CADA PRODUCTO COMPRADO AL LS */
/* Swal.fire({
	title: "Lista de Productos",
	html: `<h3>Lista de Materias</h3>
	<ul>${carrito.map(producto => `<li> -${producto.titulo}-${producto.precio}-</li>`).join("")}</ul>`,
  ...
  /* FALTA INICIALIZAR PRODUCTOS DEL CARRITO Y CONTADOR */
